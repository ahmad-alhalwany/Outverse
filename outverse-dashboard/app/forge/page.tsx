...
function genreLabel(key: string, t: (k: string) => string) {
  const g = GENRES.find((g) => g.key === key);
  return g ? t(g.labelKey) : key;
}
...
function StoryForgeContent() {
  ...
  return (
    <WorldShell colors={C} maxWidth="max-w-6xl">
      ...
      >
        ...
        <div className="relative">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            >
              <PlusIcon className="h-4 w-4" /> {t('forge.startStory')}
            </button>
          </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold" style={{ color: C.text }}>
                    {t('forge.activeStories')}
                  </p>
                  <p className="text-xs mt-1" style={{ color: C.text2 }}>
                    {t('forge.activeStoriesHint')}
                  </p>
                </div>
              ...
              <div className="feed-tabs__scroll mt-4 flex gap-1 overflow-x-auto border-b pb-1" style={{ borderColor: C.line }}>
                {TABS.map((tabDef) => (
                  <button
                    ...
                    style={{ color: tab === tabDef.key ? C.brown : C.text2, background: tab === tabDef.key ? C.white : 'transparent' }}
                  >
                    {t(tabDef.labelKey)}
                    {tab === tabDef.key && <motion.div layoutId="forgeTab" className="absolute inset-x-3 -bottom-1 h-0.5 rounded-full" style={{ background: C.brown }} />}
                  </button>
                ))}
              </div>
              ...
              {tab === 'my' && (
                <div className="feed-tabs__scroll mt-3 flex gap-2 overflow-x-auto pb-1">
                  {([
                    ...
                    ['owned', 'forge.myOwned'],
                    ['saved', 'forge.mySaved'],
                    ['collaborating', 'forge.myCollaborating'],
                ...
                <button type="button" onClick={() => openStory(spotlightStory.id)} className="mt-3 block w-full text-left">
                  ...
                  <div className="mt-4 flex items-center justify-between text-xs" style={{ color: C.text2 }}>
                    <span className="inline-flex items-center gap-1.5">
                      <UsersIcon className="h-4 w-4" /> {spotlightStory.contributors_count} {t('forge.activeSuffix')}
                    </span>
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="mt-3 flex items-center justify-between text-xs" style={{ color: C.text2 }}>
                    <span>{s.contributors_count} {t('forge.contributors')}</span>
                    <span>{s.segment_count} {t('forge.parts')}</span>
                  </div>
                </div>
              </motion.button>
            ))}
      ...
      {loading ? (
        <div className="text-center py-16" style={{ color: C.text2 }}>{t('forge.loadingStories')}</div>
      ) : loadError ? (
        <div className="rounded-2xl p-10 text-center mt-6" style={{ background: C.card2, border: `1px solid ${C.line}` }}>
          <p className="font-semibold mb-2" style={{ color: C.text }}>{t('forge.couldNotLoad')}</p>
          <button
            type="button"
            onClick={() => load()}
          ...
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm" style={{ color: C.text2 }}>
                {t('forge.storyBoardHint')}
              </p>
            </div>
            <span className="rounded-full px-3 py-1 text-xs font-semibold" style={{ background: C.card2, color: C.brown }}>
              {filteredStories.length} {t('forge.storiesCount')}
            </span>
          </div>
      ...
      <button type="button" onClick={onOpen} className="text-left flex flex-col flex-1">
        ...
        <div className="p-4 flex flex-col flex-1">
          <div className="flex items-center gap-2 mb-3">
            <span className="px-2.5 py-1 rounded-full text-xs font-medium" style={{ background: C.card2, color: C.brown }}>{genreLabel(story.genre, t)}</span>
          ...
          <div className="mt-4">
            ...
            <div className="flex items-center justify-between text-xs mt-2" style={{ color: C.text2 }}>
              <span>{story.segment_count}/{story.max_segments} {t('forge.parts')}</span>
              <span className="flex items-center gap-1"><UsersIcon className="h-3.5 w-3.5" /> {story.contributors_count} {t('forge.activeSuffix')}</span>
            </div>
          </div>
...
function EditStoryModal({ story, onClose, onSaved }: { story: Story; onClose: () => void; onSaved: () => void }) {
  ...
  return (
      ...
      <motion.form initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 20 }} onClick={(e) => e.stopPropagation()} onSubmit={submit} className="w-full max-w-lg rounded-2xl p-6 relative max-h-[90vh] overflow-y-auto" style={{ background: C.cream, boxShadow: C.modalShadow, border: `1px solid ${C.line}` }}>
        ...
        <h2 className="text-lg font-semibold mb-4" style={{ color: C.text }}>{t('forge.editStory')}</h2>
        <label className="text-sm font-medium" style={{ color: C.text2 }}>{t('forge.titleLabel')}</label>
        ...
        <label className="text-sm font-medium" style={{ color: C.text2 }}>{t('forge.descriptionLabel')}</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <select value={genre} onChange={(e) => setGenre(e.target.value)} className="w-full rounded-xl px-3 py-2.5 mt-1 outline-none" style={field}>
            {GENRES.filter((g) => g.key !== 'all').map((g) => <option key={g.key} value={g.key}>{t(g.labelKey)}</option>)}
          </select>
        ...
        <button type="submit" disabled={saving} className="mt-5 w-full rounded-xl py-3 font-semibold text-white disabled:opacity-60" style={{ background: `linear-gradient(90deg, ${C.brown}, ${C.brownDk})` }}>
          {saving ? t('forge.saving') : t('forge.saveChanges')}
        </button>
      </motion.form>
    </motion.div>
  );
...
function DeleteStoryDialog({ story, onClose, onDeleted }: { story: Story; onClose: () => void; onDeleted: () => void }) {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setError(t('forge.couldNotDelete'));
    } finally {
      setDeleting(false);
    }
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[1000] flex items-center justify-center p-4" style={{ background: C.overlay }} onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.94, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 20 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-2xl p-6" style={{ background: C.cream, border: `1px solid ${C.line}`, boxShadow: C.modalShadow }}>
        <h2 className="text-lg font-semibold" style={{ color: C.text }}>{t('forge.deleteStoryTitle')}</h2>
        <p className="text-sm mt-2" style={{ color: C.text2 }}>{t('forge.deleteStoryBody', { title: story.title })}</p>
        {error && <div className="text-sm mt-3" style={{ color: '#c0392b' }}>{error}</div>}
        <div className="flex gap-3 mt-5">
          ...
          <button type="button" onClick={() => void confirmDelete()} disabled={deleting} className="flex-1 rounded-xl py-3 text-sm font-semibold text-white" style={{ background: C.brownDk }}>
            {deleting ? t('forge.deleting') : t('common.delete')}
          </button>
        </div>
      </motion.div>
    </motion.div>
...
function CreateStoryModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <button type="button" onClick={onClose} className="absolute top-3 right-3 w-9 h-9 rounded-full text-xl flex items-center justify-center" style={{ background: C.card, color: C.text }} aria-label="Close">×</button>
        <h2 className="text-lg font-semibold mb-4" style={{ color: C.text }}>{t('forge.startNewStory')}</h2>

        <label className="text-sm font-medium" style={{ color: C.text2 }}>{t('forge.titleLabel')}</label>
        <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-xl px-3 py-2.5 mt-1 mb-3 outline-none" style={field} placeholder={t('forge.titlePlaceholder')} />

        <label className="text-sm font-medium" style={{ color: C.text2 }}>{t('forge.openingLine')}</label>
        <textarea value={premise} onChange={(e) => setPremise(e.target.value)} rows={3} className="w-full rounded-xl px-3 py-2.5 mt-1 mb-3 outline-none resize-none" style={field} placeholder={t('forge.openingLinePlaceholder')} />

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-sm font-medium" style={{ color: C.text2 }}>{t('forge.genreLabel')}</label>
            <select value={genre} onChange={(e) => setGenre(e.target.value)} className="w-full rounded-xl px-3 py-2.5 mt-1 outline-none" style={field}>
          ...
          <div>
            <label className="text-sm font-medium" style={{ color: C.text2 }}>{t('forge.maxPartsLabel')}</label>
            <input value={maxSegments} onChange={(e) => setMaxSegments(e.target.value.replace(/\D/g, ''))} className="w-full rounded-xl px-3 py-2.5 mt-1 outline-none" style={field} placeholder={t('forge.maxPartsPlaceholder')} inputMode="numeric" />
          </div>
        </div>

        <label className="text-sm font-medium mt-3 block" style={{ color: C.text2 }}>{t('forge.toneLabel')}</label>
        <input value={tone} onChange={(e) => setTone(e.target.value)} className="w-full rounded-xl px-3 py-2.5 mt-1 outline-none" style={field} placeholder={t('forge.tonePlaceholder')} />

        <div className="grid grid-cols-2 gap-3 mt-3">
          <div>
            <label className="text-sm font-medium" style={{ color: C.text2 }}>{t('forge.visibilityLabel')}</label>
          ...
          <label className="flex items-end gap-2 pb-2 text-sm" style={{ color: C.text2 }}>
            ...
            {t('forge.requireApproval')}
          </label>
        </div>

        <label className="text-sm font-medium mt-3 block" style={{ color: C.text2 }}>{t('forge.coverImageUrl')}</label>
        <input value={coverUrl} onChange={(e) => setCoverUrl(e.target.value)} className="w-full rounded-xl px-3 py-2.5 mt-1 outline-none" style={field} placeholder={t('forge.coverImageUrlPlaceholder')} />

        {error && <div className="text-sm mt-3" style={{ color: '#c0392b' }}>{error}</div>}

        <button type="submit" disabled={saving} className="mt-5 w-full rounded-xl py-3 font-semibold text-white disabled:opacity-60" style={{ background: `linear-gradient(90deg, ${C.brown}, ${C.brownDk})` }}>
          {saving ? t('forge.forging') : t('forge.forgeTheStory')}
        </button>
      </motion.form>
    </motion.div>
  );
