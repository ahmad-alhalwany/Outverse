...
export default function HomePageClient({
  ...
  useEffect(() => {
    const nextFeed = searchParams.get('feed') === 'following' ? 'following' : 'all';
    setFeed(nextFeed);
  }, [searchParams]);
  ...
  const fetchPosts = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setPosts(Array.isArray(data) ? (data as ApiPost[]) : []);
    } catch {
      setPosts([]);
      setError(true);
    } finally {
  ...
  return (
    <main className="min-h-screen bg-background text-text home-feed">
      <Header />
      <div className="home-page-shell home-app-shell app-shell pt-20 max-w-[1440px] mx-auto flex w-full min-w-0">
        <Sidebar />
